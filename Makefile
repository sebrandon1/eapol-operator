...
# eapol.openshift.io/eapol-operator-bundle:$VERSION and eapol.openshift.io/eapol-operator-catalog:$VERSION.
IMAGE_TAG_BASE ?= eapol.openshift.io/eapol-operator
...
.PHONY: shellcheck
shellcheck: ## Run shellcheck on critical shell scripts
	$(MAKE) -C images/eapol-authenticator/scripts "$@"
...
controller-gen: ## Download controller-gen locally if necessary.
	@[ -f $(CONTROLLER_GEN) ] || GOBIN=$(PROJECT_DIR)/bin go install sigs.k8s.io/controller-tools/cmd/controller-gen
...
envtest: ## Download envtest-setup locally if necessary.
	@[ -f $(ENVTEST) ] || GOBIN=$(PROJECT_DIR)/bin go install sigs.k8s.io/controller-runtime/tools/setup-envtest
